package com.phasmidsoftware.dsaipg.huskySort.sort.huskySortUtils;
...
public class ChineseCharacter extends UnicodeCharacter {
    ...
    protected String getAlt() {
        ...
        if (pinyinStrings == null) {
            final String pinyinSubstitute = String.valueOf(unicode);
            cachedPinyin = pinyinSubstitute;
            return pinyinSubstitute;
        } else if (pinyinStrings.length > 0) {
            final String pinyin = getPinyin(pinyinStrings[0]);
            cachedPinyin = pinyin;
            return pinyin;
        } else throw new RuntimeException("no pinyin available for: " + unicode);
    }

    private static String getPinyin(final String pinyinString) {
        final String pinyin = ColonPattern.matcher(pinyinString).replaceAll("~");
     ...
     */
    public static int getShift(final int property) {
     ...
     */
    public static long lookupPinyin(final int property, final String s) {
        if (s == null) return 0L;
     ...
     * @return an array of String, each element for the form "initial-final-tone"
     */
    public static <X> X[] parsePinyin(final Class<X> clazz, final String s, final int n, final Function<String[], X> function) {
    ...
    public static String toTokens(final String[] strings) {
