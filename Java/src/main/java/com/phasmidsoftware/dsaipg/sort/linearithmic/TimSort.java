...
/**
 * This is a clone of java.util.TimSort where the only changes are for instrumentation purposes.
 * Significantly, the sort method has had its signature changed so that a Helper is passed in instead of a Comparator.
 * NOTE at present, this version of TimSort does NOT do instrumentation.
 *
...
class TimSort<T> {
    ...
    private static <T> void binarySort(T[] a, int lo, int hi, int start, Helper<T> helper) {
        ...
        if (start == lo)
            start++;
        for (; start < hi; start++) {
            T pivot = instrumented ? helper.get(a, start) : a[start];
