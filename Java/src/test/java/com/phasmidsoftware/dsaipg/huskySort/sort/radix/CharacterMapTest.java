...
import com.phasmidsoftware.dsaipg.huskySort.sort.huskySortUtils.UnicodeCharacter;
...
public class CharacterMapTest {
    ...
    public void testGet1() {
        assertEquals(0, characterMap.size());
        final String x = "阿朝";
        ...
        assertEquals(2, characterMap.size());
        final UnicodeCharacter unicodeCharacter2 = characterMap.get(x.charAt(0));
        assertEquals(2, characterMap.size());
        assertEquals(unicodeCharacter2, characterMap.get(x.charAt(0)));
    }
    ...
    public ChineseCharacter getChineseCharacterWithCounter(final Character ch, final MyInteger chars) {
        final ChineseCharacter result = new ChineseCharacter(ch);
    ...
    public void testGet2() {
        final MyInteger chars = new MyInteger(0);
        final CharacterMap myCharacterMap = new CharacterMap(ch -> getChineseCharacterWithCounter(ch, chars));
        assertEquals(0, myCharacterMap.size());
        final String x = "阿朝";
        final UnicodeCharacter unicodeCharacter0 = myCharacterMap.get(x.charAt(0));
        assertEquals(unicodeCharacter0, myCharacterMap.get(x.charAt(0)));
        assertEquals(1, chars.value);
        final UnicodeCharacter unicodeCharacter1 = myCharacterMap.get(x.charAt(1));
        assertEquals(unicodeCharacter1, myCharacterMap.get(x.charAt(1)));
        assertEquals(2, chars.value);
        assertEquals(2, myCharacterMap.size());
        final UnicodeCharacter unicodeCharacter2 = myCharacterMap.get(x.charAt(0));
        assertEquals(unicodeCharacter2, myCharacterMap.get(x.charAt(0)));
        assertEquals(2, myCharacterMap.size());
        assertEquals(2, chars.value);
    }
