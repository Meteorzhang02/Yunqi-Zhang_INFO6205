...
import com.phasmidsoftware.dsaipg.huskySort.sort.Sort;
...
public class BucketSortTest {
    ...
    public void sort4() throws Exception {
        ...
        Sort<Integer> sorter = new BucketSort<>(2, helper);
    ...
    public void sortN() throws Exception {
        ...
        ComparableSortHelper<Integer> helper = new ComparableSortHelper<>("BucketSort", xs.length);
        Sort<Integer> sorter = new BucketSort<>(100, helper);
        Integer[] ys = sorter.sort(xs);
        assertTrue(helper.sorted(ys));
        System.out.println(sorter.toString());
    }
    ...
    public void sortEmpty() throws Exception {
        final Integer[] xs = new Integer[0];
        ComparableSortHelper<Integer> helper = new ComparableSortHelper<>("BucketSort", xs.length);
        Sort<Integer> sorter = new BucketSort<>(5, helper);
        Integer[] ys = sorter.sort(xs);
        assertTrue(helper.sorted(ys));
    }
    ...
    public void sortSingleElement() throws Exception {
        final Integer[] xs = {7};
        ComparableSortHelper<Integer> helper = new ComparableSortHelper<>("BucketSort", xs.length);
        Sort<Integer> sorter = new BucketSort<>(3, helper);
        Integer[] ys = sorter.sort(xs);
        assertTrue(helper.sorted(ys));
    }
}